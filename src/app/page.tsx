	...
	return (
		<main className="flex min-h-screen bg-gray-900 flex-col items-center justify-between px-24 py-12">
			...
			<div className="mb-4 text-center">
				This is a project that uses the ElevenLabs API to convert your text into a speech dictated by a fake,
				AI-generated voice.
			</div>
			<div className="w-full items-center flex flex-col gap-2 mt-2">
						...
						{arrayOfVoices ? (
							<div className="flex gap-4">
								{arrayOfVoices.map((e: VoiceObject) => (
									<button
										className={`border ${
											theVoiceId === e.voice_id ? "bg-blue-700" : ""
										}   px-2 my-2 rounded-lg  py-1`}
				...
				<textarea
					className=" p-1 w-[40rem] text-black rounded-md  border-gray-600"
					onChange={(event) => {
				...
				<button
					className="bg-green-600 px-5 mt-2 py-1 self-center h-max rounded-md hover:bg-green-900 disabled:cursor-not-allowed disabled:bg-blue-900 transition-colors"
					onClick={generateSpeech}
					disabled={isLoading || !theText}>
					{isLoading ? "Loading..." : "Generate Voice"}
				</button>
			</div>
